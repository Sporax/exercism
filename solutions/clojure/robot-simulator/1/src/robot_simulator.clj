...
(defn advance [state]
  (case (state :bearing)
    :north (assoc-in state [:coordinates :x] (- (get-in state [:coordinates :x]) 1))
    :west (assoc-in state [:coordinates :y] (- (get-in state [:coordinates :y]) 1))
    :south (assoc-in state [:coordinates :x] (+ (get-in state [:coordinates :x]) 1))
    :east (assoc-in state [:coordinates :y] (+ (get-in state [:coordinates :y]) 1))))
